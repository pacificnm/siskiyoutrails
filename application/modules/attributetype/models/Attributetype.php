<?php
class Attributetype_Model_Attributetype extends Application_Model_Model
{
	...
	public function get($search, $pagination = false, $page = 1)
	{
		...
		$select->order('attribute_type_value');
	...
	public function update ($id, $data)
	{
		try {
			$where = $this->getTable()
			->getDefaultAdapter()
			->quoteInto('attribute_type_id = ?', $id);
	...
	protected function getTable ()
	{
		if (null !== $this->_table) {
			...
			$this->_table = new Attributetype_Model_DbTable_Attributetype();
			return $this->_table;
		}
	}
}
